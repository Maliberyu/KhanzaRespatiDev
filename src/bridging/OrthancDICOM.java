...
public class OrthancDICOM extends javax.swing.JDialog {
	...
	BtnVideo = new widget.Button();
BtnVideo.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/save-16x16i.png"))); // Ganti dengan icon video yang sesuai
BtnVideo.setMnemonic('V');
BtnVideo.setText("Download Video");
BtnVideo.setToolTipText("Alt+V");
BtnVideo.setName("BtnVideo");
BtnVideo.setPreferredSize(new java.awt.Dimension(100, 30));
BtnVideo.addActionListener(new java.awt.event.ActionListener() {
    public void actionPerformed(java.awt.event.ActionEvent evt) {
        BtnVideoActionPerformed(evt);
    }
});
PanelMenu.add(BtnVideo);
	...
	private void BtnVideoActionPerformed(java.awt.event.ActionEvent evt) {
    // Pastikan Anda memiliki metode untuk mengambil video dari server atau API Orthanc
    orthanc.AmbilVideo(norawat, series);
}
...
//                @Override
public void AmbilVideo(String norawat, String series) {
    // Tentukan URL atau endpoint untuk mengunduh video
    String videoUrl = "https://serverorthanc.com/download/video?norawat=" + norawat + "&series=" + series;
    // Tentukan path untuk menyimpan file video
    String pathToSave = "hasilradiologi/video_" + norawat + "_" + series + ".mp4"; // Anda bisa menyesuaikan format video

    try {
        // Membuat folder jika tidak ada
        java.nio.file.Path path = Paths.get("hasilradiologi");
        if (!Files.exists(path)) {
            Files.createDirectories(path);  // Membuat direktori jika belum ada
        }

        // Logika untuk mengunduh video
        URL url = new URL(videoUrl);
        InputStream in = url.openStream();
        Files.copy(in, Paths.get(pathToSave), StandardCopyOption.REPLACE_EXISTING);
        JOptionPane.showMessageDialog(null, "Video berhasil diunduh dan disimpan di folder hasilradiologi", "Download Sukses", JOptionPane.INFORMATION_MESSAGE);
    } catch (IOException e) {
        JOptionPane.showMessageDialog(null, "Terjadi kesalahan saat mengunduh video: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
    }
}
...
private widget.Button BtnVideo	;
