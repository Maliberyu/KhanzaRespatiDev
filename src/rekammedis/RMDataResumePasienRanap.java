...
public final class RMDataResumePasienRanap extends javax.swing.JDialog {
    ...
    private String jenisBayarPasien;
    private String namaBangsalPasien;
...
//            }
    ...
    private void MnLaporanResumeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnLaporanResumeActionPerformed
        if(tbObat.getSelectedRow()>-1){
            ...
            param.put("finger","https://rsiarespati-tsm.simkeskhanza.com/qr/?id="+EnkripsiAES.encrypt("{'x':'ri','t':'"+TNoRw.getText()+"'}"));
            try {
...
//        KdRuang.setText(kdbangsal);
//        NmRuang.setText(nmbangsal);
...
//    DTPCari2.setDate(tgl2); 
    ...
    try {
        // 1. Check if this is a merged treatment patient
        String no_rawat_yang_diquery = norwt;  // Changed from no_rawat to norwt (parameter name)
        
        PreparedStatement cekRawatGabung = koneksi.prepareStatement(
            "SELECT ranap_gabung.no_rawat2 FROM ranap_gabung WHERE ranap_gabung.no_rawat = ?");
        cekRawatGabung.setString(1, norwt);  // Changed from no_rawat to norwt
        ResultSet hasilCek = cekRawatGabung.executeQuery();
        
        if (hasilCek.next()) {
            no_rawat_yang_diquery = hasilCek.getString("no_rawat2");
        }
        hasilCek.close();
        cekRawatGabung.close();
        
        // 2. Get payment type and room data
        PreparedStatement ambilData = koneksi.prepareStatement(
            "SELECT penjab.png_jawab, bangsal.nm_bangsal " +
            "FROM reg_periksa " +
        ...
        ambilData.setString(1, no_rawat_yang_diquery);
        ResultSet hasilData = ambilData.executeQuery();
        
        if (hasilData.next()) {
            // Store the values in class variables
            this.jenisBayarPasien = hasilData.getString("png_jawab");
            this.namaBangsalPasien = hasilData.getString("nm_bangsal");
            
            // Uncomment these if you have components to display the data
            // if (txtCaraBayar != null) txtCaraBayar.setText(this.jenisBayarPasien);
            // if (txtNamaBangsal != null) txtNamaBangsal.setText(this.namaBangsalPasien);
        }
        hasilData.close();
        ambilData.close();
    } catch (Exception e) {
        System.out.println("Error saat mengambil data tambahan: " + e.getMessage());
    }
    // =============================================
    // AKHIR BAGIAN BARU
    // =======================
    
    // KODE ASLI TETAP DIJAGA
    isRawat();              
    ChkInput.setSelected(true);
    isForm();
    ...
    public void isCek(){
       ...
       if(akses.getjml2()>=1){
           KodeDokter.setText(akses.getkode());
            NamaDokter.setText(pegawai.tampil3(KodeDokter.getText()));
...
//        }            
    ...
    public void setNoRm(String string, Date date) {
