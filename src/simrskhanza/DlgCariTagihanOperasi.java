...
//));
                    ...
                    try {
                        try {
                            // Cek apakah no_rawat adalah rawat gabung (bayi), dan ambil rawat ibunya
                            String noRawatPilihan = tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString();
                                String noRawatIbu = Sequel.cariIsi(
                                    "SELECT no_rawat FROM ranap_gabung WHERE no_rawat2 = ?",
                                    noRawatPilihan
                                );

                                if (noRawatIbu.isEmpty()) {
                                    noRawatIbu = noRawatPilihan;  // Yang dipilih adalah ibu
                                }
//            String noRawatIbu = Sequel.cariIsi(
//                "SELECT no_rawat FROM ranap_gabung WHERE no_rawat2 = ?",
...
//
//            if (noRawatIbu.isEmpty()) {
//                noRawatIbu = tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString();
//            }
                            rs = koneksi.prepareStatement(
                                    "SELECT no_rawat, tgl_perawatan, jam_rawat, suhu_tubuh, tensi, nadi, respirasi, tinggi, berat, gcs, keluhan, pemeriksaan, alergi, " +
                                    "rtl, penilaian FROM pemeriksaan_ranap WHERE no_rawat = '" + noRawatIbu + "' " +
...
//                                    "order by tgl_perawatan, jam_rawat desc limit 1").executeQuery();
                           ...
                           param.put("ruang", Sequel.cariIsi(
                                ...
                                "AND nm_bangsal NOT LIKE '%PERINA%' " +
                                "AND nm_bangsal NOT LIKE '%BAYI%' " +
                                "ORDER BY kamar_inap.tgl_masuk ASC LIMIT 1",
                                noRawatIbu
                            ));
